...
def get_block():
    #定义全局变量
    global transactions, support_ratio, frequency
...
def run_apriori(mode='common', k=0):
    get_block()
    ...
    if mode == 'common':
        for key, value in result.items():
            one_patterns = sorted(value, key=lambda x: x[1], reverse=True)
...
tk.Button(root, text="运行Apriori算法,并按照频繁项集次序输出", command=lambda: run_apriori('common')).pack()
tk.Button(root, text="运行Apriori算法,并按照支持度次序输出", command=lambda: run_apriori('sort')).pack()
tk.Button(root, text="运行Apriori算法,并输出极大频繁项集", command=lambda: run_apriori('maximal')).pack()
tk.Button(root, text="运行Apriori算法,并输出支持度前k项", command=lambda: run_apriori('topk')).pack()
